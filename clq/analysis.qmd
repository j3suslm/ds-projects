...
![](header.png){width=65% .lightbox fig-align="left"}

Several violent episodes in Mexico suggest a worrying trend (The Economist)
...
df4 = results.to_pandas()
```

```{python}
#| label: tbl-combined
#| tbl-cap: Preview of comined dataset
(
  df4
      .head(10)
      .style.hide(axis='index')
...
# convert to pandas
df_pd = results.to_pandas().round(2)

# concat tables
df_geo = (
    mexico.merge(df_pd, on='cve_geo', how='left')
...
The application of Location Quotient (LQ) methodology to crime analysis is a specialized area within spatial criminology, drawing heavily on its original use in economic geography. The literature establishes LQ as a powerful tool for moving beyond simple crime rates to identify areas where specific criminal activity is disproportionately concentrated.
